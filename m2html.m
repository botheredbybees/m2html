...
for i=1:2:length(paramlist)
	...
	switch(optionsnames{ind})
		...
		case {'globalhypertextlinks', 'global'}
			if strcmpi(pvalue,'on')
		...
		case 'project'
			if ischar(pvalue)
				options.project = pvalue;
			else
				error(sprintf(msgInvalidPair,pname));
			end
		case 'version'
			if ischar(pvalue)
				options.version = pvalue;
			else
				error(sprintf(msgInvalidPair,pname));
			end
		case 'ignoreddir'
...
for i=1:length(mdir)
	for j=1:length(mdirs)
		if strcmp(mdirs{j},mdir{i})
			...
			for k=1:length(ind)
				if ~strcmp(names{ind(k)}, names{j}) % ignore calls to self
			...
			for k=1:length(ind)
				if ~strcmp(names{ind(k)}, names{j}) % ignore calls from self
					if strcmp(mdirs{j},mdirs{ind(k)})
			...
			if ~isempty(subroutine{j}) & options.source
				for k=1:length(subroutine{j})
					tpl = set(tpl, 'var', 'L_SUB', ['#_sub' num2str(k)-1]);
					tpl = set(tpl, 'var', 'SUB',   subroutine{j}{k});
					tpl = set(tpl, 'var', 'SUBH1',   subroutineH1{j}{k});
